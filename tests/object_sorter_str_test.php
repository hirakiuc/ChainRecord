...
class ObjectSorterStrTest extends PHPUnit_Framework_TestCase {
    ...
    public function testIntSort_with_one_key(){
        ...
        for($i=0;$i<count($ret)-1;$i++){ 
            $this->assertTrue( strcmp($ret[$i]->prop1, $ret[$i+1]->prop1) <= 0 );
        }
    }
    ...
    public function testIntSort_with_two_key(){
        ...
        for($i=0;$i<count($ret)-1;$i++){ 
            ...
            if($ret[$i]->prop1 === $ret[$i]->prop2){
                $this->assertTrue( strcmp($ret[$i]->prop2, $ret[$i+1]->prop2) <= 0);
            }
        }
    }
